...
void fillPolygonWithAsterisks() {
    glColor3f(0.0, 1.0, 0.0); // Green color
    for (float x = 2 * scaleFactor; x < 10 * scaleFactor; x += 2) {
        for (float y = 4 * scaleFactor; y < 12 * scaleFactor; y += 2) {
            renderBitmapString(x, y, GLUT_BITMAP_HELVETICA_12, "*");
        }
    }
}
...
void display() {
    ...
    // Fill polygon with green asterisks
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("OpenGL Filled Polygon with Green Asterisks");
    init();
